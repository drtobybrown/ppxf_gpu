...
import sys
import os
# Add parent directory to path to allow importing ppxf as a package
# We use insert(0, ...) to ensure this takes precedence over the current directory
...
def benchmark_sdss(n_iterations=10):
    ...
    ppxf_dir = os.path.dirname(os.path.abspath(__file__))
    filename = os.path.join(ppxf_dir, 'spectra/NGC3522_SDSS_DR18.fits')
    ...
    model_filename = os.path.join(ppxf_dir, 'sps_models', basename)
